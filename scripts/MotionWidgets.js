 ...
 *
 **/
var MotionLayerWidget = function(elem, opts, layers) {
    this.opts = {
      cols: 12,
      rows: 6,
      selectLimit: 0,
...
MotionLayerWidget.prototype = {
    ...
    _makeSelectable: function() {
      ...
      this.elem.selectable({
        ...
        selecting : function(e, ui) {
          $(ui.selecting).removeClass( function(i, css) {
            return (css.match (/\bmot_bg[0-9]+/g) || []).join(' ');
          }).addClass(me.bgcls[3]);
        },
      ...
      $('.mot_td').css("width", (720 / o.cols) + "px");
    },
    _create: function() {
      var self=this;
      var o=this.opts;
      ...
      for( var r=0 ; r < o.rows ; r++ ) {
        ...
        for( var c=0 ; c < o.cols ; c++ ) {
          tdobj = $('<td class="mot_td mot_bg0"></td>')
            .prop('id', 'line' + r + '_' + c)
            .data('idx', r * o.cols + c)
            .appendTo(trobj);
        }
...
/**
 *
 * Widget for Motion Widget Cell
 *
 **/
var MotionCellWidget = function(elem, opts) {
    this.opts = {
      cols: 12,
      rows: 6,
      mam: 0,
...
MotionCellWidget.prototype = {
    ...
    _makeSelectable: function() {
      ...
      this.elem.selectable({
        filter : 'td',
        selecting : function(e, ui) {
          var pos = me._getCellPos(ui.selecting);

          if ( dragval == '2' ) {
              dragval = $(ui.selecting).data('val');
          }
        ...
        unselecting : function(e, ui) {
          var val = $(ui.unselecting).data('val');
          var pos = me._getCellPos(ui.unselecting);
        ...
        stop : function(e, ui) {
          ...
          for( var i=0; i<selected.length ; i++ ) {
            var idx = parseInt($(selected[i]).data('idx'));
            me.area = me.area.setCharAt(idx, val);
          }

          selected = [];

          if( me.prevarea == me.area ) {
            me.updateMblockAll('0');
          } else {
            data['area'] = me.area;
            o.callback(data);
            me.updateMblock(data);
          }
    ...
    _updateMblockReverse: function() {
        var len = this.opts.cols * this.opts.rows;
        var area = this.area;
        var data = {};
        ...
        for(var i=0 ; i < len ; i++ ) {
            if( area.charAt(i) == '0' ) {
                area = area.setCharAt(i, '1');                
            } else {
                area = area.setCharAt(i, '0');
            }
        }
